...
6. **Linked stackydo tasks.** Look for tasks the user mentioned by short_id or by topic. If the user has open `idea` or `triage` items in the relevant area, surface them and ask which to attach. For each attached task, add the `sprint-<slug>` tag (no leading date) via `mcp__plugin_delivery-workflow_stackydo__update_task`.
